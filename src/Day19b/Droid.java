...
public class Droid {
    ...
    public void run() {
        ...
        while (!found) {
            ...
            int startX = startY * 10 / 14;

            while (!lineStartFound) {
                startX++;
                coors.addLast(startX);
                coors.addLast(startY);
                new VM(program, this).runProgram();
                lineStartFound = output == 1;
            }

            coors.addLast(startX + 99);
            coors.add(startY - 99);
            new VM(program, this).runProgram();

            if (output == 1) {
                System.out.println("Start point at " + startX + ":" + (startY - 99));
                found = true;
            } else {
                startY++;
            }
