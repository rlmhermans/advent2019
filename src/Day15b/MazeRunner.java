...
public class MazeRunner {
    ...
    //north (1), south (2), west (3), and east (4)
    private void processMove(boolean found) {
        switch (direction) {
            case 1:
                botY--;
                direction = 4;
        ...
        maze[botY][botX] = found ? 2 : 1;
        if (found) {
            oxX = botX;
            oxY = botY;
        }

        if (startX == botX && startY == botY) {
    ...
    public void findShortestPath() {
        ...
        points.add(new Point(oxX, oxY, 0));
        ...
        while (!points.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (!visited[next.y][next.x] && (maze[next.y][next.x] == 1 || maze[next.y][next.x] == 2)) {
        ...
        for(Point p : visitedPoints) {
            if(p.distanceFromStart > longestPath) longestPath = p.distanceFromStart;
        }

        System.out.println("It takes this much minutes: " + longestPath);
    }
