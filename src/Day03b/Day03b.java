...
//        String[] wireTwoInstructions = {"L10", "U10", "R10", "U5"};
        ...
        for (Point p : intersectionPoints) {
            int combinedPathLength = getStepsToPoint(firstWireParts, p) + getStepsToPoint(secondWireParts, p);
            if (combinedPathLength < shortest) {
                shortest = combinedPathLength;
            }
        }
    ...
    private static int getStepsToPoint(ArrayList<Line> wireParts, Point target) {
        int counter = 0;

        for (Line l : wireParts) {
            if (l.getStartX() == l.getEndX()) {
                if (l.getStartY() < l.getEndY()) {
                    for (int i = l.getStartY(); i < l.getEndY(); i++) {
                        if (target.getX() == l.getStartX() && target.getY() == i) {
                            return counter;
                        }
                        counter++;
                    }
                ...
                if (l.getStartX() < l.getEndX()) {
                    for (int i = l.getStartX(); i < l.getEndX(); i++) {
                        if (target.getY() == l.getStartY() && target.getX() == i) {
                            return counter;
                        }
                        counter++;
                    }
    ...
    private static ArrayList<Line> generateWireParts(String[] wireInstructions) {
        int currentX = 0;
        int currentY = 0;
        ArrayList<Line> lines = new ArrayList<>();

        for (String s : wireInstructions) {
            String direction = s.substring(0, 1);
            int distance = Integer.parseInt(s.substring(1));

            switch (direction) {
                case "L":
                    lines.add(new Line(currentX, currentY, currentX - distance, currentY));
                    currentX -= distance;
                    break;
                case "R":
                    lines.add(new Line(currentX, currentY, currentX + distance, currentY));
                    ...
                    break;
                case "U":
                    lines.add(new Line(currentX, currentY, currentX, currentY + distance));
                    currentY += distance;
                    break;
                case "D":
                    lines.add(new Line(currentX, currentY, currentX, currentY - distance));
