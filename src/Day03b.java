...
public class Day03b {
    public static void main(String[] args) {
        ...
        for (Line l1 : firstWireParts) {
            for (Line l2 : secondWireParts) {
                if (l1.intersects(l2.getBoundsInLocal())) {
                    ArrayList<Point2D> l1Points = generateLinePoint2Ds(l1);
                    ArrayList<Point2D> l2Points = generateLinePoint2Ds(l2);
                    l1Points.retainAll(l2Points);
                    intersectionPoints.addAll(l1Points);
                }
            }
        }
        ...
        for (int i = 1; i < intersectionPoints.size(); i++) {
            ...
            int distance = (int)(Math.abs(point.getX()) + Math.abs(point.getY()));
    ...
    public static ArrayList<Line> generateWireParts(String[] wireInstructions) {
        int currentX = 0;
        int currentY = 0;
    ...
    public static ArrayList<Point2D> generateLinePoint2Ds(Line l) {
        ArrayList<Point2D> points = new ArrayList<>();

        int startX = (int) l.getStartX();
        int startY = (int) l.getStartY();
        int endX = (int) l.getEndX();
        int endY = (int) l.getEndY();

        if (startX < endX) {
            for (int i = startX; i <= endX; i++) {
                points.add(new Point2D(i, startY));
            }
        } else if (startY < endY) {
            for (int i = startY; i <= endY; i++) {
                points.add(new Point2D(startX, i));
            }
        } else if (startX > endX) {
